...
import { cruiseDestinationsAPI, mediaAPI, imageUrl } from "../api/api";

export default function AddCruiseDestinationPage() {
  ...
  useEffect(() => {
    if (!isEdit) return;
    setLoading(true);
    cruiseDestinationsAPI.get(id)
      .then(res => {
        const d = res.data;
        if (!d) { navigate("/cruise-destinations"); return; }
        setForm({
          name: d.name ?? "",
          country: d.country ?? "",
      ...
      .catch(() => navigate("/cruise-destinations"))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await cruiseDestinationsAPI.update(id, payload);
      } else {
        await cruiseDestinationsAPI.create(payload);
      }
      navigate("/cruise-destinations");
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={isEdit ? "Edit Cruise Destination" : "Add Cruise Destination"}
        subtitle={isEdit ? "Update cruise destination details" : "Add a new cruise destination to the platform"}
      />
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <FormInput label="Destination Name *" placeholder="e.g. Mediterranean Cruise"
            value={form.name} onChange={e => updateForm("name", e.target.value)} />
          <FormInput label="Country *" placeholder="e.g. Greece & Italy"
        ...
        <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
          <button type="button" onClick={() => navigate("/cruise-destinations")}
          ...
          <button type="button" onClick={handleSave} disabled={saving || uploading}
            ...
            {saving ? "Saving…" : isEdit ? "Update Cruise Destination" : "Save Cruise Destination"}
          </button>
        </div>
      </div>
    </div>
