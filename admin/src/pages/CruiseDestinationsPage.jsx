...
import { Plus, Edit, Trash2, MapPin, Anchor } from "lucide-react";
import PageHeader from "../components/ui/PageHeader";
import SearchInput from "../components/ui/SearchInput";
import DeleteModal from "../components/ui/DeleteModal";
import Pagination from "../components/ui/Pagination";
import { cruiseDestinationsAPI, imageUrl } from "../api/api";
...
export default function CruiseDestinationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await cruiseDestinationsAPI.list(params);
  ...
  const handleDelete = async () => {
    try { await cruiseDestinationsAPI.delete(deleteTarget); load(); }
    catch (e) { console.error(e); }
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Cruise Destinations" subtitle={`${total} destinations`}
        action={
          <Link to="/cruise-destinations/add" className="flex items-center gap-2 bg-teal-600 hover:bg-teal-700 text-white font-semibold text-sm px-5 py-2.5 rounded-xl transition-colors">
            <Plus className="w-4 h-4" /> Add Cruise Destination
          </Link>
        }
      />
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-teal-600 font-medium flex items-center gap-1">
                      ...
                      {Number(dest.cruise_count) || 0} cruises
                    </span>
                    <div className="flex gap-2">
                      <Link to={`/cruise-destinations/${dest.id}/edit`}
      ...
      <DeleteModal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete}
        title="Delete Cruise Destination" message="This will permanently delete the cruise destination." />
    </div>
  );
}
