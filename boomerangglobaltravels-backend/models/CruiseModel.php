...
class CruiseModel
{
    ...
    public function count(array $filters = []): int
    {
        ...
        $stmt = $this->db->prepare("SELECT COUNT(*) FROM cruises p LEFT JOIN cruise_destinations d ON p.cruise_destination_id = d.id WHERE p.deleted_at IS NULL {$where}");
        $stmt->execute($params);
    ...
    public function all(array $filters = [], int $limit = 10, int $offset = 0, string $sort = 'created_at', string $order = 'desc'): array
    {
        ...
        $stmt = $this->db->prepare(
            "SELECT p.id, p.title, p.slug, p.category, p.duration, p.starting_price,
                    ...
                    p.created_at, p.updated_at, p.cruise_destination_id,
                    d.name AS destination_name,
                    d.region AS destination_region
             FROM cruises p
             LEFT JOIN cruise_destinations d ON p.cruise_destination_id = d.id
    ...
    public function create(array $data): int
    {
        $stmt = $this->db->prepare(
            'INSERT INTO cruises
             (title, slug, category, cruise_destination_id, duration, starting_price, rating, review_count, status, featured,
        ...
        $stmt->execute([
            ...
            $data['slug'],
            $data['category']              ?? null,
            $data['cruise_destination_id'] ?? null,
            $data['duration']              ?? null,
    ...
    public function update(int $id, array $data): bool
    {
        ...
        $allowed = ['title', 'slug', 'category', 'cruise_destination_id', 'duration', 'starting_price', 'rating', 'review_count',
        ...
        $stmt = $this->db->prepare("UPDATE cruises SET {$fieldsSql} WHERE id = ? AND deleted_at IS NULL");
        $stmt->execute($params);
    ...
    {
        $stmt = $this->db->prepare('UPDATE cruises SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND deleted_at IS NULL');
        $stmt->execute([$id]);
        return $stmt->rowCount() > 0;
    }
    ...
    public function toggleFeatured(int $id): ?int
    {
        ...
        $stmt = $this->db->prepare('UPDATE cruises SET featured = ? WHERE id = ? AND deleted_at IS NULL');
    ...
    private function buildWhere(array $filters): array
    {
        ...
        if (!empty($filters['search'])) {
            $where   .= ' AND (p.title LIKE ? OR p.category LIKE ? OR d.name LIKE ?)';
            $term     = '%' . $filters['search'] . '%';
            $params[] = $term;
            $params[] = $term;
            $params[] = $term;
