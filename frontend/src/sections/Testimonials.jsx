        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((t, i) => (
            <motion.div key={t.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
              ...
              <div className="absolute top-5 right-5 opacity-10"><Quote className="w-12 h-12 text-teal-600" /></div>
              <div className="flex gap-0.5 mb-4">
                {Array.from({ length: t.rating }).map((_, si) => <Star key={si} className="w-4 h-4 fill-amber-400 text-amber-400" />)}
              </div>
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-amber-100">
                {t.avatar_url ? (
                  <img src={t.avatar_url} alt={t.customer_name} className="w-12 h-12 rounded-full object-cover ring-2 ring-amber-100" />
                ) : (
                  <div className="w-12 h-12 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 font-bold shrink-0">{t.customer_name?.[0]}</div>
                )}
                <div>
                  <div className="font-bold text-gray-900 text-sm">{t.customer_name}</div>
                  <div className="text-gray-400 text-xs">{t.customer_location}</div>
                  <div className="text-teal-600 text-xs font-medium mt-0.5">{t.package_name}</div>
                </div>
              </div>
            </motion.div>
          ))}
